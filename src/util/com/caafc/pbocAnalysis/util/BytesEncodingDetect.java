...
public class BytesEncodingDetect extends Encoding {
	...
	public static void main(String[] args){
		String fileName = "D:/我的文档/Workspaces/MyEclipse 8.5/CustomSpider2011/src/bean/PageNode.java";
		String charset  = BytesEncodingDetect.getJavaEncode(fileName);
	...
	public int detectEncoding(URL testurl) {
		...
		try {
			...
			while ((bytesread = chinesestream
					.read(rawtext, byteoffset, rawtext.length - byteoffset)) > 0) {
			...
			if (rawtext[i] >= 0) {
				...
				if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xF7
					...
					if (GBKFreq[row][column] != 0) {
						...
						&& rawtext[i + 1] <= (byte) 0x39 && (byte) 0x81 <= rawtext[i + 2]
						&& rawtext[i + 2] <= (byte) 0xFE && (byte) 0x30 <= rawtext[i + 3]
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			if (rawtext[i] >= 0) { // in ASCII range
				...
				if (i + 3 < rawtextlen && (byte) 0x8E == rawtext[i]
						...
						&& (byte) 0xA1 <= rawtext[i + 3] && rawtext[i + 3] <= (byte) 0xFE) { // Planes
																								// 1
																								// -
																								// 16
					cnschars++;
			...
			if (rawtext[i] == (byte) 0x1B && i + 3 < rawtextlen) { // Escape
				...
				if (rawtext[i + 1] == (byte) 0x24 && rawtext[i + 2] == 0x29
					...
					while (rawtext[i] != (byte) 0x1B) {
						...
						&& rawtext[i + 2] == (byte) 0x29 && rawtext[i + 3] == (byte) 0x47) {
					// CNS Escape $ ) G
					i += 4;
					while (rawtext[i] != (byte) 0x1B) {
						dbchars++;
				...
				if (rawtext[i] == (byte) 0x1B && i + 2 < rawtextlen
						&& rawtext[i + 1] == (byte) 0x28 && rawtext[i + 2] == (byte) 0x42) { // ASCII:
					// ESC
					// ( B
					i += 2;
				}
			...
			if (rawtext[i] >= 0) {
				...
				if ((byte) 0x81 <= rawtext[i]
						...
						&& ((byte) 0x41 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x5A
								|| (byte) 0x61 <= rawtext[i + 1] && rawtext[i + 1] <= (byte) 0x7A || (byte) 0x81 <= rawtext[i + 1]
								&& rawtext[i + 1] <= (byte) 0xFE)) {
					krchars++;
					totalfreq += 500;
					if ((byte) 0xA1 <= rawtext[i] && rawtext[i] <= (byte) 0xFE
