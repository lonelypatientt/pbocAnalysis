...
import com.skyon.core.util.ArrayUtil;
...
public class StringUtil
{
	 ...
	 * @return
	 */
	public static boolean isNotNullByCustom(String str){
		if(str==null||str.trim().equals("")||str.trim().equals("1")||str.trim().equals("无")||str.trim().equals("空")||str.trim().equals("null")||str.trim().equals("NULL")||str.trim().equals("--")||str.trim().equals("－－")){
			return false;
		}
		return true;
	}
	...
	public static String emptyToNull(String noNullString)
	{
		if(noNullString.length() > 0)
		{
			...
			{
				if (noNullString.charAt(i) != ' ') return noNullString;
			}
		}
		return null;	
	}
	 ...
	 * @return 原来的空值转变为指定值, 
	 */
	public static String safeNotNull(String string, String defaltValue)
	{
		return (string!=null) ? string : defaltValue ;
	}
	/**
	 * 
	 * @param s
	 ...
	 * 
	 * @param s
	 * @return 将 String 的 首个字符换成小写
	 */
	static public final String lowCaseFirstChar(String s)
	{
	 ...
	 * 
	 * @param s
	 * @return 将 String 的 首个字符换成大写
	 */
	static public final String upperCaseFirstChar(String s)
	{
	   if (s==null || s.length()==0 ) {return s;}
...
//	}
	...
	/** @param src
	 * @return 将 GBK 转化为 Unicode Ascii */
	static public final String gbkToUnicodeSytle(String src)
	{
	 ...
	 * @return 将 8859-1 的文本 转换 到GBK 上 
	 */
	static public final String ANSIStringToGBKString(String ansiString)
	{
		if (ansiString ==null) return null ;
		byte[] bs;
		try
		{
	 ...
	 * @return Url
	 */
	public  static String URL_ANSIEncode(String msg) 
	{
		try
		{
    ...
    /** 将按javaName 排列的字符串, 按大写字符分割成下划线的连接的字符串, 连续的大写字符认为是一个单词 
     * 例如:  openSource => open_source 
     *        userID => user_id 
     *        userIDFind => user_id_find 
     *        UserIERFindD = user_ier_find_d
     * @param javaName 
     * @return 按大写字符分割成下划线的连接的字符串,
     * */
    public static final String convertJavaName2ULWord(String javaName)
    {
        ...
        for (int i = 0; i < javaName.length(); i++)
        {
            ...
            boolean isLowerBefore = (i==0?true:Character.isLowerCase(javaName.charAt(i-1)));
            boolean isLowerAfter = (i==javaName.length()-1?false:Character.isLowerCase(javaName.charAt(i+1)));
            
            if (Character.isUpperCase(c)  && (isLowerBefore || isLowerAfter )) 
            {
                //new word
                if (p1!=0) {t.append("_");}
                t.append(javaName.substring(p1,i).toLowerCase()); 
                p1 = i ;
            } 
        }
        if (p1!=0) {t.append("_");}
        t.append(javaName.substring(p1).toLowerCase()); 
        return t.toString();
    }
     ...
     *  
     *  
     *  {@link #convertJavaName2ULWord} 的逆变换
     */
    public static final String convertULWord2JavaName(String ulName)
    {

        StringBuffer  result = new StringBuffer(ulName.length());
        for (StringTokenizer st = new StringTokenizer(ulName,"_"); st.hasMoreTokens();)
        {
           String tmp = st.nextToken();

           result.append(tmp.substring(0, 1).toUpperCase());
           result.append(tmp.substring(1).toLowerCase());
        }
        return result.toString() ;
    }
	 ...
	 */
	public static String replaceEnvVar(Properties varMapping,  String str) throws IllegalArgumentException
	{
		StringBuffer buffer = new StringBuffer();
		int p ,start=0;
		while ( (p = str.indexOf("${",start)) >=0)
		{
			if (start<p)
				buffer.append(str.substring(start,p));
			int p1 = str.indexOf("}", p);
			if (p1<0)
			{
				throw new IllegalArgumentException("miss symbol '}'");
			}
			String key = str.substring(p+2,p1);
			String values = varMapping.getProperty(key);
			if (values!=null)
			{
